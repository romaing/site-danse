...
async function setupComplete() {
  console.log('🚀 CONFIGURATION COMPLÈTE DU SITE DE DANSE\n');

  // Étape 1: Import des professeurs
  ...
  const professeursIds = [];
  for (const prof of professeurs) {
    try {
      const result = await apiRequest('/api/professeurs', 'POST', { data: prof });
      professeursIds.push({ id: result.data.id, nom: prof.nom, prenom: prof.prenom, specialites: prof.specialites });
  ...
  for (const stage of stages) {
    try {
      ...
      stagesIds.push({ id: result.data.id, titre: stage.titre });
  ...
  for (const seance of seances) {
    try {
      const result = await apiRequest('/api/seances', 'POST', { data: seance });
      seancesIds.push({ id: result.data.id, titre: seance.titre, type_danse: seance.type_danse });
  ...
  // Étape 4: Associations automatiques
  console.log('\n🔗 ÉTAPE 4: Associations professeurs-séances...');

  // Fonction pour trouver les professeurs par spécialité
  function findProfesseursBySpecialite(specialite) {
    return professeursIds.filter(prof =>
      prof.specialites.some(spec =>
        spec.toLowerCase().includes(specialite.toLowerCase()) ||
        specialite.toLowerCase().includes(spec.toLowerCase().split(' ')[0])
      )
    ).map(prof => prof.id);
  }

  for (const seance of seancesIds) {
    const profIds = findProfesseursBySpecialite(seance.titre);
    if (profIds.length > 0) {
      try {
        await apiRequest(`/api/seances/${seance.id}`, 'PUT', {
          data: { professeurs: profIds }
        });
        console.log(`✅ Séance "${seance.titre}" → ${profIds.length} professeur(s)`);
      } catch (error) {
        console.log(`❌ Association séance ${seance.id}:`, error.message);
      }
    }
  }

  console.log('\n🎭 ÉTAPE 5: Associations professeurs-stages...');

  // Associations prédéfinies pour les stages
  const stageAssociations = {
    "Stage Danse & Ski à Pralognan": ["Schlienger", "Galichet"], // Jonathan + Stéphane
    "Stage de Danse de Salon à Royan": ["Grecias", "Desjardins"], // Céline + Sophie
    "Stage Multi-danses à Laguiole": ["Schlienger", "Galichet", "Mbani", "Grecias", "Desjardins", "Lasnier"], // Tous
    "Stage de Pâques - Vallée de l'Yonne": ["Desjardins", "Lasnier"] // Sophie + Marie-France
  };

  for (const stage of stagesIds) {
    const profNoms = stageAssociations[stage.titre];
    if (profNoms) {
      const profIds = professeursIds
        .filter(prof => profNoms.includes(prof.nom))
        .map(prof => prof.id);
      ...
      if (profIds.length > 0) {
        try {
          ...
          console.log(`✅ Stage "${stage.titre}" → ${profIds.length} professeur(s)`);
        } catch (error) {
          console.log(`❌ Association stage ${stage.id}:`, error.message);
        }
      }
    }
  }

  // Étape 6: Publier tous les contenus
  console.log('\n📢 ÉTAPE 6: Publication des contenus...');

  // Publier professeurs
  for (const prof of professeursIds) {
    try {
      await apiRequest(`/api/professeurs/${prof.id}`, 'PUT', {
        data: { publishedAt: new Date().toISOString() }
      });
    } catch (error) {
      // Ignore les erreurs de publication
    }
  }
  ...
  console.log('📊 RÉSUMÉ:');
  console.log(`   - ${professeursIds.length} professeurs avec spécialités`);
  console.log(`   - ${stagesIds.length} stages 2026`);
  console.log(`   - ${seancesIds.length} séances de danse`);
  console.log('   - Toutes les associations configurées');
  console.log('   - Contenus publiés');
  console.log('\n🔗 Accès:');
  console.log('   Admin: http://localhost:1337/admin');
  console.log('   Site: http://localhost:3001');
  console.log('\n✨ Le site de danse est maintenant 100% opérationnel !');
}
